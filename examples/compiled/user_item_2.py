...
async def buy_item_step_2(ctx: StatefulFunction, params, attr_1 = None, reply_to: list = None):
    ...
    if reply_to is None:
        reply_to = []
    reply_to.append({'op_name': 'user', 'fun': 'buy_item_step_3', 'id': ctx.key, 'context': {'amount': amount, 'total_price': total_price, 'item': item, 'attr_1': attr_1}})
...
async def bulk_purchase_with_tiers(ctx: StatefulFunction, cart: list[str], quantities: list[int], reply_to: list = None) -> bool:
    state = ctx.get()
    total_cost = 0
    state['__loop_iter_1'] = iter(range(len(cart)))
    ctx.put(state)
...
async def bulk_purchase_with_tiers_step_2(ctx: StatefulFunction, params, placeholder_return = None, reply_to: list = None):
    ...
    try:
        index = next(state['__loop_iter_1'])
    ...
    if reply_to is None:
        reply_to = []
    reply_to.append({'op_name': 'user', 'fun': 'bulk_purchase_with_tiers_step_3', 'id': ctx.key, 'context': {'cart': cart, 'total_cost': total_cost, 'requested_amount': requested_amount, 'item': item, 'index': index, 'quantities': quantities}})
...
async def bulk_purchase_with_tiers_step_3(ctx: StatefulFunction, params, attr_4 = None, reply_to: list = None):
    ...
    if attr_4 >= requested_amount:
        current_item_cost = 0
        state['__loop_iter_2'] = iter(range(1, requested_amount + 1))
        ctx.put(state)
...
async def bulk_purchase_with_tiers_step_4(ctx: StatefulFunction, params, placeholder_return = None, reply_to: list = None):
    ...
    try:
        unit = next(state['__loop_iter_2'])
        
        # Nested Conditionals: The more you buy, the cheaper the marginal unit gets
        if unit > 50:
            if reply_to is None:
                reply_to = []
            reply_to.append({'op_name': 'user', 'fun': 'bulk_purchase_with_tiers_step_6', 'id': ctx.key, 'context': {'cart': cart, 'total_cost': total_cost, 'requested_amount': requested_amount, 'item': item, 'index': index, 'quantities': quantities, 'current_item_cost': current_item_cost, 'unit': unit, 'attr_4': attr_4}})
            ctx.call_remote_async(operator_name = 'item', function_name = 'get_price', key = item, params = (reply_to,))
        elif unit > 10:
            if reply_to is None:
                reply_to = []
            reply_to.append({'op_name': 'user', 'fun': 'bulk_purchase_with_tiers_step_7', 'id': ctx.key, 'context': {'cart': cart, 'total_cost': total_cost, 'requested_amount': requested_amount, 'item': item, 'index': index, 'quantities': quantities, 'attr_1': attr_1, 'current_item_cost': current_item_cost, 'unit': unit, 'attr_4': attr_4}})
            ctx.call_remote_async(operator_name = 'item', function_name = 'get_price', key = item, params = (reply_to,))
        else:
            if reply_to is None:
                reply_to = []
            reply_to.append({'op_name': 'user', 'fun': 'bulk_purchase_with_tiers_step_8', 'id': ctx.key, 'context': {'cart': cart, 'total_cost': total_cost, 'attr_2': attr_2, 'requested_amount': requested_amount, 'item': item, 'index': index, 'quantities': quantities, 'attr_1': attr_1, 'current_item_cost': current_item_cost, 'unit': unit, 'attr_4': attr_4}})
            ctx.call_remote_async(operator_name = 'item', function_name = 'get_price', key = item, params = (reply_to,))
    except StopIteration:
        ...
        if (total_cost + current_item_cost) > state['balance']:
            ...
            ctx.put(state)
            ctx.call_remote_async(operator_name = 'user', function_name = 'bulk_purchase_with_tiers_step_2', key = ctx.key, params = ({'attr_4': attr_4, 'cart': cart, 'current_item_cost': current_item_cost, 'index': index, 'item': item, 'quantities': quantities, 'requested_amount': requested_amount, 'total_cost': total_cost, 'unit': unit}, None, reply_to))
        else:
            if reply_to is None:
                reply_to = []
            reply_to.append({'op_name': 'user', 'fun': 'bulk_purchase_with_tiers_step_5', 'id': ctx.key, 'context': {'cart': cart, 'total_cost': total_cost, 'requested_amount': requested_amount, 'item': item, 'index': index, 'quantities': quantities, 'current_item_cost': current_item_cost, 'unit': unit, 'attr_4': attr_4}})
...
async def bulk_purchase_with_tiers_step_7(ctx: StatefulFunction, params, attr_2 = None, reply_to: list = None):
    state = ctx.get()
    (attr_1, attr_4, cart, current_item_cost, index, item, quantities, requested_amount, total_cost, unit) = (params['attr_1'], params['attr_4'], params['cart'], params['current_item_cost'], params['index'], params['item'], params['quantities'], params['requested_amount'], params['total_cost'], params['unit'])
    current_item_cost = current_item_cost + int(attr_2 * 0.9)
    ctx.put(state)
    ctx.call_remote_async(operator_name = 'user', function_name = 'bulk_purchase_with_tiers_step_4', key = ctx.key, params = ({'attr_1': attr_1, 'attr_2': attr_2, 'attr_4': attr_4, 'cart': cart, 'current_item_cost': current_item_cost, 'index': index, 'item': item, 'quantities': quantities, 'requested_amount': requested_amount, 'total_cost': total_cost, 'unit': unit}, None, reply_to))

@user_operator.register
async def bulk_purchase_with_tiers_step_8(ctx: StatefulFunction, params, attr_3 = None, reply_to: list = None):
    state = ctx.get()
    (attr_1, attr_2, attr_4, cart, current_item_cost, index, item, quantities, requested_amount, total_cost, unit) = (params['attr_1'], params['attr_2'], params['attr_4'], params['cart'], params['current_item_cost'], params['index'], params['item'], params['quantities'], params['requested_amount'], params['total_cost'], params['unit'])
