...
async def bulk_purchase_with_tiers_step_2(ctx: StatefulFunction, params, placeholder_return = None, reply_to: list = None):
    ...
    if state['__loop_index_1'] >= len(cart):
        ...
        if reply_to is None:
            reply_to = []
        reply_to.append({'op_name': 'user', 'fun': 'bulk_purchase_with_tiers_step_3', 'id': ctx.key, 'context': {'quantities': quantities, 'requested_amount': requested_amount, 'cart': cart, 'total_cost': total_cost, 'item': item, 'index': index}})
...
async def bulk_purchase_with_tiers_step_4(ctx: StatefulFunction, params, placeholder_return = None, reply_to: list = None):
    ...
    if state['__loop_index_2'] >= requested_amount + 1:
        ...
        if reply_to is None:
            reply_to = []
        reply_to.append({'op_name': 'user', 'fun': 'bulk_purchase_with_tiers_step_5', 'id': ctx.key, 'context': {'quantities': quantities, 'requested_amount': requested_amount, 'cart': cart, 'total_cost': total_cost, 'current_item_cost': current_item_cost, 'item': item, 'attr_4': attr_4, 'index': index}})
        ...
        if unit > 50:
            if reply_to is None:
                reply_to = []
            reply_to.append({'op_name': 'user', 'fun': 'bulk_purchase_with_tiers_step_6', 'id': ctx.key, 'context': {'quantities': quantities, 'requested_amount': requested_amount, 'cart': cart, 'total_cost': total_cost, 'current_item_cost': current_item_cost, 'item': item, 'attr_4': attr_4, 'unit': unit, 'index': index}})
            ctx.call_remote_async(operator_name = 'item', function_name = 'get_price', key = item, params = (reply_to,))
        elif unit > 10:
            if reply_to is None:
                reply_to = []
            reply_to.append({'op_name': 'user', 'fun': 'bulk_purchase_with_tiers_step_7', 'id': ctx.key, 'context': {'quantities': quantities, 'requested_amount': requested_amount, 'cart': cart, 'total_cost': total_cost, 'current_item_cost': current_item_cost, 'item': item, 'attr_4': attr_4, 'unit': unit, 'index': index}})
            ctx.call_remote_async(operator_name = 'item', function_name = 'get_price', key = item, params = (reply_to,))
        else:
            if reply_to is None:
                reply_to = []
            reply_to.append({'op_name': 'user', 'fun': 'bulk_purchase_with_tiers_step_8', 'id': ctx.key, 'context': {'quantities': quantities, 'requested_amount': requested_amount, 'cart': cart, 'total_cost': total_cost, 'current_item_cost': current_item_cost, 'item': item, 'attr_4': attr_4, 'unit': unit, 'index': index}})
            ctx.call_remote_async(operator_name = 'item', function_name = 'get_price', key = item, params = (reply_to,))
    ctx.put(state)
