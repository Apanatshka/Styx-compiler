...
async def buy_item(ctx: StatefulFunction, amount: int, item: str, reply_to: list = None) -> bool:
    ...
    if reply_to is None:
        reply_to = []
    reply_to.append(
        {"op_name": "user", "fun": "buy_item_step_2", "id": ctx.key, "context": {"item": item, "amount": amount}}
    )
    ...
    reply_to.append(
        {
            ...
            "fun": "buy_item_step_3",
            "id": ctx.key,
            "context": {"item": item, "attr_1": attr_1, "amount": amount, "total_price": total_price},
        }
    )
...
async def bulk_purchase_with_tiers(
    ...
    ctx.call_remote_async(
        ...
        key=ctx.key,
        params=({"cart": cart, "quantities": quantities, "total_cost": total_cost}, None, reply_to),
    )
    ctx.put(state)
...
async def bulk_purchase_with_tiers_step_2(
    ctx: StatefulFunction, params, placeholder_return=None, reply_to: list = None
):
    state = ctx.get()
    ...
    if state["__loop_index_1"] >= len(cart):
        ...
        if reply_to:
            ...
            ctx.call_remote_async(
                ...
                params=(
                    reply_info["context"],
                    "Bulk purchase complete. Remaining balance: " + str(state["balance"]),
                    reply_to,
                ),
            )
            return None
    ...
    reply_to.append(
        {
            "op_name": "user",
            "fun": "bulk_purchase_with_tiers_step_3",
            "id": ctx.key,
            "context": {
                "item": item,
...
async def bulk_purchase_with_tiers_step_3(ctx: StatefulFunction, params, attr_4=None, reply_to: list = None):
    state = ctx.get()
    (cart, index, item, quantities, requested_amount, total_cost) = (
        params["cart"],
        params["index"],
        params["item"],
        params["quantities"],
...
async def bulk_purchase_with_tiers_step_4(
    ...
    state = ctx.get()
    (attr_4, cart, current_item_cost, index, item, quantities, requested_amount, total_cost) = (
        params["attr_4"],
        params["cart"],
    ...
    if state["__loop_index_2"] >= requested_amount + 1:
        ...
        reply_to.append(
            {
                "op_name": "user",
                "fun": "bulk_purchase_with_tiers_step_5",
                "id": ctx.key,
                "context": {
                    "item": item,
                    "requested_amount": requested_amount,
        ...
        if unit > 50:
            ...
            reply_to.append(
                {
                    "op_name": "user",
                    "fun": "bulk_purchase_with_tiers_step_6",
                    "id": ctx.key,
                    "context": {
                        "item": item,
                        "requested_amount": requested_amount,
            ...
            reply_to.append(
                {
                    "op_name": "user",
                    "fun": "bulk_purchase_with_tiers_step_7",
                    "id": ctx.key,
                    "context": {
                        "item": item,
                        "requested_amount": requested_amount,
            ...
            reply_to.append(
                {
                    "op_name": "user",
                    "fun": "bulk_purchase_with_tiers_step_8",
                    "id": ctx.key,
                    "context": {
                        "item": item,
                        "requested_amount": requested_amount,
...
async def bulk_purchase_with_tiers_step_6(ctx: StatefulFunction, params, attr_1=None, reply_to: list = None):
    state = ctx.get()
    (attr_4, cart, current_item_cost, index, item, quantities, requested_amount, total_cost, unit) = (
        params["attr_4"],
        params["cart"],
        params["current_item_cost"],
        params["index"],
