...
async def test_stack(ctx: StatefulFunction, user: str, amount: int, reply_to: list = None) -> bool:
    ...
    if reply_to is None:
        reply_to = []
    reply_to.append({'op_name': 'item', 'fun': 'test_stack_step_2', 'id': ctx.key, 'context': {'amount': amount, 'user': user}})
    ctx.call_remote_async(operator_name = 'user', function_name = 'buy_item', key = user, params = (amount, ctx.key, reply_to))
...
async def buy_item_step_2(ctx: StatefulFunction, params, attr_1 = None, reply_to: list = None):
    ...
    if reply_to is None:
        reply_to = []
    reply_to.append({'op_name': 'user', 'fun': 'buy_item_step_3', 'id': ctx.key, 'context': {'amount': amount, 'total_price': total_price, 'attr_1': attr_1, 'item': item}})
...
async def create_user_item_step_3(ctx: StatefulFunction, params, placeholder_return = None, reply_to: list = None):
    ...
    if reply_to:
        reply_info = reply_to.pop()
        ctx.call_remote_async(operator_name = reply_info["op_name"], function_name = reply_info["fun"], key = reply_info["id"], params = (reply_info["context"], new_item, reply_to))
        return
    else:
        return new_item
...
async def test_loop_step_2(ctx: StatefulFunction, params, placeholder_return = None, reply_to: list = None):
    ...
    if state['__loop_index_1'] >= amount:
        ...
        if reply_to is None:
            reply_to = []
        reply_to.append({'op_name': 'user', 'fun': 'test_loop_step_2', 'id': ctx.key, 'context': {'amount': amount, 'i': i, 'item': item}})
        ctx.call_remote_async(operator_name = 'item', function_name = 'update_stock', key = item, params = (-1, reply_to))
    ctx.put(state)


@user_operator.register
async def process_inventory(ctx: StatefulFunction, budget: int, items: list[str], reply_to: list = None) -> str:
    state = ctx.get()
    total_spent = 0
...
async def process_inventory_step_2(ctx: StatefulFunction, params, placeholder_return = None, reply_to: list = None):
    ...
    if state['__loop_index_1'] >= len(items):
        ...
        if reply_to:
            reply_info = reply_to.pop()
            ctx.call_remote_async(operator_name = reply_info["op_name"], function_name = reply_info["fun"], key = reply_info["id"], params = (reply_info["context"], "New balance: " + str(state['balance']), reply_to))
            return
        else:
        ...
        if reply_to is None:
            reply_to = []
        reply_to.append({'op_name': 'user', 'fun': 'process_inventory_step_3', 'id': ctx.key, 'context': {'item': item, 'items': items, 'budget': budget, 'total_spent': total_spent}})
        ctx.call_remote_async(operator_name = 'item', function_name = 'get_price', key = item, params = (reply_to,))
    ctx.put(state)
...
async def process_inventory_step_3(ctx: StatefulFunction, params, price = None, reply_to: list = None):
    ...
    if reply_to is None:
        reply_to = []
    reply_to.append({'op_name': 'user', 'fun': 'process_inventory_step_4', 'id': ctx.key, 'context': {'items': items, 'budget': budget, 'item': item, 'total_spent': total_spent, 'price': price}})
...
async def process_inventory_step_4(ctx: StatefulFunction, params, stock = None, reply_to: list = None):
    state = ctx.get()
    (budget, item, items, price, total_spent) = (params['budget'], params['item'], params['items'], params['price'], params['total_spent'])
    ...
    if price < budget:
        if stock > 10:
            if reply_to is None:
                reply_to = []
            reply_to.append({'op_name': 'user', 'fun': 'process_inventory_step_5', 'id': ctx.key, 'context': {'items': items, 'budget': budget, 'item': item, 'stock': stock, 'total_spent': total_spent, 'price': price}})
            ctx.call_remote_async(operator_name = 'item', function_name = 'update_stock', key = item, params = (-5, reply_to))
        elif stock > 0:
            if reply_to is None:
                reply_to = []
            reply_to.append({'op_name': 'user', 'fun': 'process_inventory_step_6', 'id': ctx.key, 'context': {'stock': stock, 'items': items, 'budget': budget, 'total_spent': total_spent, 'price': price, 'item': item}})
            ctx.call_remote_async(operator_name = 'item', function_name = 'update_stock', key = item, params = (-1, reply_to))
        else:
        ...
        if reply_to is None:
            reply_to = []
        reply_to.append({'op_name': 'user', 'fun': 'process_inventory_step_7', 'id': ctx.key, 'context': {'stock': stock, 'items': items, 'budget': budget, 'total_spent': total_spent, 'price': price, 'item': item}})
        ctx.call_remote_async(operator_name = 'item', function_name = 'update_stock', key = item, params = (1, reply_to))

@user_operator.register
async def process_inventory_step_5(ctx: StatefulFunction, params, placeholder_return = None, reply_to: list = None):
    state = ctx.get()
    (budget, item, items, price, stock, total_spent) = (params['budget'], params['item'], params['items'], params['price'], params['stock'], params['total_spent'])
    total_spent = total_spent + price
    logging.warn(f"Total spent so far: {total_spent}")
    state['myitems'].append(item)
    ctx.put(state)
...
@user_operator.register
async def nested_loop_test(ctx: StatefulFunction, amount: int, items: list[str], reply_to: list = None) -> str:
    state = ctx.get()
    total = 0
    state['__loop_index_1'] = 0
    ctx.put(state)
...
async def nested_loop_test_step_2(ctx: StatefulFunction, params, placeholder_return = None, reply_to: list = None):
    ...
    if state['__loop_index_1'] >= len(items):
        ...
        if reply_to is None:
            reply_to = []
        reply_to.append({'op_name': 'user', 'fun': 'nested_loop_test_step_3', 'id': ctx.key, 'context': {'amount': amount, 'item': item, 'items': items, 'total': total}})
        ctx.call_remote_async(operator_name = 'item', function_name = 'get_price', key = item, params = (reply_to,))
    ctx.put(state)
...
async def nested_loop_test_step_4(ctx: StatefulFunction, params, placeholder_return = None, reply_to: list = None):
    ...
    if state['__loop_index_2'] >= amount:
        ...
        if reply_to is None:
            reply_to = []
        reply_to.append({'op_name': 'user', 'fun': 'nested_loop_test_step_4', 'id': ctx.key, 'context': {'total': total, 'items': items, 'amount': amount, 'price': price, 'i': i, 'item': item}})
        ctx.call_remote_async(operator_name = 'item', function_name = 'update_stock', key = item, params = (-1, reply_to))
    ctx.put(state)
