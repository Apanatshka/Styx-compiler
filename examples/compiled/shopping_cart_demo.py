...
async def find(ctx: StatefulFunction, reply_to: list = None):
    ...
    if reply_to:
        ...
        ctx.call_remote_async(
            ...
            function_name=reply_info["fun"],
            key=reply_info["id"],
            params=(reply_info["context"], {"stock": state["stock"], "price": state["price"]}, reply_to),
        )
        return None
...
async def add_credit(ctx: StatefulFunction, amount: int, reply_to: list = None):
    ...
    if reply_to:
        ...
        ctx.call_remote_async(
            ...
            function_name=reply_info["fun"],
            key=reply_info["id"],
            params=(reply_info["context"], {"credit": state["credit"]}, reply_to),
        )
        return None
...
async def remove_credit(ctx: StatefulFunction, amount: int, reply_to: list = None):
    ...
    if state["credit"] - amount < 0:
        attr_1 = state["NotEnoughCredit"]("User does not have enough credit")
        raise attr_1
    state["credit"] -= amount
    ctx.put(state)
...
async def add_item(ctx: StatefulFunction, item: Item, quantity: int, reply_to: list = None):
    ...
    if reply_to is None:
        reply_to = []
    reply_to.append(
        {"op_name": "order", "fun": "add_item_step_2", "id": ctx.key, "context": {"quantity": quantity, "item": item}}
    )
...
async def add_item_step_2(ctx: StatefulFunction, params, price=None, reply_to: list = None):
    ...
    if reply_to:
        ...
        ctx.call_remote_async(
            ...
            function_name=reply_info["fun"],
            key=reply_info["id"],
            params=(reply_info["context"], self, reply_to),
        )
        return None
    return self
...
async def checkout(ctx: StatefulFunction, reply_to: list = None) -> str:
    ...
    ctx.call_remote_async(
        operator_name="user", function_name="remove_credit", key=state["user"], params=(state["total_cost"], reply_to)
    )
...
async def checkout_step_2(ctx: StatefulFunction, params, placeholder_return=None, reply_to: list = None):
    ...
    if reply_to:
        ...
        ctx.call_remote_async(
            ...
            function_name=reply_info["fun"],
            key=reply_info["id"],
            params=(reply_info["context"], "Payment successful", reply_to),
        )
        return None
...
async def find(ctx: StatefulFunction, reply_to: list = None):
    ...
    if reply_to:
        ...
        return None
    return {"paid": state["paid"], "items": readable_items, "user": state["user"], "total_cost": state["total_cost"]}
