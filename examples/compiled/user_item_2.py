...
async def buy_item_step_2(ctx: StatefulFunction, params, attr_1 = None, reply_to: list = None):
    ...
    if reply_to is None:
        reply_to = []
    reply_to.append({'op_name': 'user', 'fun': 'buy_item_step_3', 'id': ctx.key, 'context': {'item': item, 'attr_1': attr_1, 'amount': amount, 'total_price': total_price}})
...
async def bulk_purchase_with_tiers_step_2(ctx: StatefulFunction, params, placeholder_return = None, reply_to: list = None):
    ...
    if state['__loop_index_1'] >= len(cart):
        ...
        if reply_to is None:
            reply_to = []
        reply_to.append({'op_name': 'user', 'fun': 'bulk_purchase_with_tiers_step_3', 'id': ctx.key, 'context': {'item': item, 'requested_amount': requested_amount, 'total_cost': total_cost, 'cart': cart, 'index': index, 'quantities': quantities}})
...
async def bulk_purchase_with_tiers_step_4(ctx: StatefulFunction, params, placeholder_return = None, reply_to: list = None):
    ...
    if state['__loop_index_2'] >= requested_amount + 1:
        ...
        if (total_cost + current_item_cost) > state['balance']:
            ...
            if reply_to is None:
                reply_to = []
            reply_to.append({'op_name': 'user', 'fun': 'bulk_purchase_with_tiers_step_5', 'id': ctx.key, 'context': {'item': item, 'requested_amount': requested_amount, 'attr_4': attr_4, 'total_cost': total_cost, 'cart': cart, 'index': index, 'current_item_cost': current_item_cost, 'quantities': quantities}})
        ...
        if unit > 50:
            if reply_to is None:
                reply_to = []
            reply_to.append({'op_name': 'user', 'fun': 'bulk_purchase_with_tiers_step_6', 'id': ctx.key, 'context': {'item': item, 'requested_amount': requested_amount, 'attr_4': attr_4, 'total_cost': total_cost, 'cart': cart, 'index': index, 'unit': unit, 'current_item_cost': current_item_cost, 'quantities': quantities}})
            ctx.call_remote_async(operator_name = 'item', function_name = 'get_price', key = item, params = (reply_to,))
        elif unit > 10:
            if reply_to is None:
                reply_to = []
            reply_to.append({'op_name': 'user', 'fun': 'bulk_purchase_with_tiers_step_7', 'id': ctx.key, 'context': {'item': item, 'requested_amount': requested_amount, 'attr_4': attr_4, 'total_cost': total_cost, 'cart': cart, 'index': index, 'unit': unit, 'current_item_cost': current_item_cost, 'quantities': quantities}})
            ctx.call_remote_async(operator_name = 'item', function_name = 'get_price', key = item, params = (reply_to,))
        else:
            if reply_to is None:
                reply_to = []
            reply_to.append({'op_name': 'user', 'fun': 'bulk_purchase_with_tiers_step_8', 'id': ctx.key, 'context': {'item': item, 'requested_amount': requested_amount, 'attr_4': attr_4, 'total_cost': total_cost, 'cart': cart, 'index': index, 'unit': unit, 'current_item_cost': current_item_cost, 'quantities': quantities}})
            ctx.call_remote_async(operator_name = 'item', function_name = 'get_price', key = item, params = (reply_to,))
    ctx.put(state)
